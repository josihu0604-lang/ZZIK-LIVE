...
│   ├── api/            # API 라우트
│   │   ├── qr/verify/           # QR 검증 (4상태)
│   │   ├── verify/location/     # GPS 검증
│   │   ├── verify/complete/     # 통합 판정
│   │   └── receipts/            # 영수증 업로드/OCR
│   ├── auth/           # 인증 페이지
│   └── feed/           # 피드 (LABS 플래그로 비활성)
├── components/          # React 컴포넌트
├── lib/                 # 유틸리티 함수
│   ├── server/         # 서버 유틸 (idempotency, rate-limit)
│   ├── map/            # 지도 관련
│   └── analytics/      # 분석 도구
├── prisma/              # DB 스키마
├── tests/               # 테스트 코드
...
다음 기능들은 LABS 플래그로 제어되며 기본적으로 비활성화됩니다:

- **Feed**: `NEXT_PUBLIC_ENABLE_FEED=false` (나노 크리에이터 피드)
- **Auth Required**: `REQUIRE_AUTH_FOR_VERIFY=false` (검증 시 인증 강제)

활성화하려면 `.env` 파일에서 플래그를 `true`로 설정하세요.
